...
| **辉光强度** | 辉光亮度倍数 |
| **辉光半径** | 辉光扩散到间隙的距离 |
| **Alpha Clip** | 启用后裁剪半透明区域（适合某些渲染场景） |
...
- **Avatar 使用**：辉光在 Avatar 上也能正常工作（基于透明度叠加），但 Bloom 需看世界是否开启
- 着色器为 **透明队列**（Transparent），ZWrite 关闭，在 VRChat 中渲染顺序正常
